...
func readWeiChatBill() {
	...
	// 打开excel文件
	f, err := excelize.OpenFile("weichat.xlsx")
	if err != nil {
		log.Fatal(err)
	}
...
func readAlipayBill() {
	...
	// 打开excel文件
	f, err := excelize.OpenFile("alipay.xlsx")
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, row := range rows {
		log.Println(row[10])
		tmpTime, err := time.Parse("2006/01/02 15:04:05", row[10])
		if err != nil {
			log.Fatal(err)
		}
		tmpAmmount, err := decimal.NewFromString(row[5])
		if err != nil {
			log.Fatal(err)
		}
		tmpBill := &bill{
